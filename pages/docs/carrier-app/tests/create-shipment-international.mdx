---
title: Automated Tests | createShipment_international
description: This test is part of the ShipEngine Connect carrier app test suite. It tests your app's ability to create an international shipment.
---
...
This test exercises your app's ability to successfully create an international shipment. If your app does not have any international [delivery services](../../reference/delivery-service.mdx), then this test will be skipped by [the `connect test` command](../../cli.mdx#test-your-app).
...
<Reference>
  ...
  <Field name="deliveryServiceName" type="string" required={false}>
    ...
    <DefaultValue>
      The name of the first international delivery service defined for your app.
    </DefaultValue>
  </Field>
...
------------------------------
Here's an example of using the `connect.config.js` file to customize the `createShipment_international` test. This example provides values for all parameters and uses the global `connectArgs` parameter for authentication.
