...
[Our CLI](./cli.mdx) allows you to choose between JavaScript and TypeScript when you run the [`connect init` command](./cli.mdx#initialize-a-new-app) to [create a new app](./create-first-app.mdx). Selecting TypeScript will generate `.ts` files that import the necessary type definitions from our SDK. It will also create a [`tsconfig.json`](https://www.typescriptlang.org/docs/handbook/tsconfig-json.html), which allows you to adjust TypeScript's settings.
...
<Pager prev="./implementation.mdx" prevTitle="Implementing Your Methods" next="./error-handling.mdx" nextTitle="Error Handling" />
