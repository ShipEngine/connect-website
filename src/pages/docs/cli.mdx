...
Test Your App
--------------------------
The `connect test` command runs our [end-to-end acceptance test suite](./testing#acceptance-testing) against your app to confirm that it behaves as expected. These tests emulate real-world workflows that your app will experience in production, and you should make sure your app passes before publishing it.
...
--------------------------
The `connect env` topic allows you to manage environment variables for your published application. Environment
variables for an application are stored in an encrypted enclave suitable for any secrets integral
to running the application. For more details see [Environment Variables](environment-variables.mdx).

> **Note:** When an application is published ShipEngine Connect uses a point in time snapshot of the variables
that have been set. Modifying variables post-publish will require a re-publish for them to apply.

```
connect env:list
connect env:set   NAME=value NAME_2=value2 ... NAME_N=valueN
connect env:get   NAME=value NAME_2=value2 ... NAME_N=valueN
connect env:unset NAME=value NAME_2=value2 ... NAME_N=valueN
```
#### Options
...
##### `env:list -f, --format=table|dotenv  [default: table] specify output format`
This option allows list to be output as a table or as [dotenv](https://github.com/motdotla/dotenv) NAME=value format.
