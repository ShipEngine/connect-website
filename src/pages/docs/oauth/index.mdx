...
# Mono Auth Overview
The ShipEngine Connect platform uses an internal system called `MonoAuth` to handle Oauth 1 and 2 auth handshake flows.
...
- Define your application as using OAuth as seen in the following [example]()
- Publish your application to ShipEngine Connect using the `connect publish`
- Contact a member of the [ShipEngine Connect Team]() in order to establish a MonoAuth installation
...
## Advanced Topics
- [Response Transformation](./advanced/response-transformation.mdx) *(specify how responses from the two exchanges can be mapped into the MonoAuth canonical model)*
...
The sample data in [dev_integration_data.json](../sample-data/dev_integration_data.json)
contains some basic examples of this:
...
compute data. The simplest value resolvers in MonoAuth simply use the RHS to point to a property.
