...
to MonoAuth with a `code`. Monoauth makes a server-server request to the 3rd
party to exchange the code for an access token.
The `request_token` component of the DSL describes how MonoAuth should
construct and execute that request.

:brain: See the [OAuth 2.0 RFC - Access Token Request](https://tools.ietf.org/html/rfc6749#section-4.1.3)
