...
from the two exchanges can be mapped into the MonoAuth canonical model, the
[`IntegrationTokenResponse`](../MonoAuth/Server/Data/IntegrationTokenResponse.cs).
...
The `response` object defines a JSONPath expression that MonoAuth
...
1. `expires_at` Is an object, rather than a JSONPath.
    * The `path` property is JSONPath informing MonoAuth where to obtain the
    value.
...
:warning: `expires_in` vs `expires_at` OAuth2 specifies `expires_in` as a standard
...
of the latter, `expires_at` MonoAuth will parse the date-time and compute an
`expires_in` value.
<Pager
