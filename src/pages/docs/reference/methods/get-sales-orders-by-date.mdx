...
<Reference>
  ...
  <Field name="statusMappings" type="object" nullable={true}>
    <Description>
      A customer specified desired mapping for status codes. The keys of this object will be the custom status strings provided by a user, and the value will be a related SalesOrderStatus it should be mapped to.
    </Description>
  </Field>
...
<Reference>
 ...
 <Field name="buyer.name" type="object"  required={true}>
  ...
  <Field name="requestedFulfillments[].extensions" required={false}>
    <Type>
      object
    </Type>
    <Description>
      (Deprecated) This property is only used for legacy integrations.
    </Description>
  </Field>
