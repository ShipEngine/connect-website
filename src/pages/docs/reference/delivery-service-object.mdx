...
For example, the `shipment` argument passed to the [createShipment method](./methods/create-shipment.mdx)
...
<Reference>
  ...
  <Field name="grade" type="string" nullable={false}>
    <Description>
  ...
  <Field name="hasSandbox" type="boolean" nullable={false}>
    <Description>
      Indicates whether the carrier provides a [sandbox](./../sandbox.mdx) API for this delivery service. A sandbox should mimic real functionality as much as possible but MUST NOT incur any actual costs or affect production data.
    </Description>
  </Field>
