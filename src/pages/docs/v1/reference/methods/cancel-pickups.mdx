...
```typescript
import { Transaction, PickupCancellation, PickupCancellationOutcome } from '@shipengine/connect'
...
<Field name="pickupService" type="object" nullable={false}>
  <Description>The [pickup service](./../pickup-service.mdx) to use for this pickup request.</Description>
</Field>
...
<Field name="pickupService.id" nullable={false}>
  <Type>[UUID](https://www.npmjs.com/package/uuid)</Type>
  <Description>
    A UUID that uniquely identifies the pickup service. This ID should never change. This is the UUID you used in the
...
<Field name="pickupService.code" type="string" nullable={false}>
  <Description>Optional code used to map to what the carrier uses to identify the pickup service.</Description>
</Field>
...
<Field name="pickupService.name" type="string" nullable={false}>
  <Description>
    The user-friendly service name (e.g. "One-Time Pickup", "Recurring Pickup", "Drop-Off"). This string will not
...
<Field name="reason" type="string" nullable={false}>
  <Description>
    The reason for the cancellation. Valid values include the following: * `not_ready` - The package was not ready for
    pickup. * `price` - The customer cancelled the pickup because of the price. * `schedule` - The pickup was cancelled
    because it could not be picked up within the needed time sframe. * `carrier_failed_pickup` - The carrier failed to
    pick up the package. * `other` - The cancellation is for a reason not covered by any of the other categories.
  </Description>
</Field>
...
<Field name="notes" type="object[]" nullable={false}>
  <Description>An array of objects containing additional information about this cancellation.</Description>
</Field>
...
<Field name="address" nullable={false}>
  ...
  <Description>The address where the package(s) should be picked up.</Description>
</Field>

{' '}

<Field name="contact" nullable={false}>
  <Type>[contact info object](./../contact-info.mdx)</Type>
  <Description>An object representing contact information about the person there to meet the driver.</Description>
</Field>
...
<Field name="shipments[].packages[].packaging.id" nullable={false}>
  <Type>[UUID](https://www.npmjs.com/package/uuid)</Type>
  <Description>
    A UUID that uniquely identifies this packaging. This is the UUID you used int he [Packaging
    Definition](./../packaging.mdx) file for this packaging.
  </Description>
</Field>
...
<Field name="shipments[].packages[].packaging[].code" type="string" nullable={false}>
  <Description>Optional code used to map to what the carrier uses to identify the packaging.</Description>
</Field>
...
<Field name="shipments[].packages[].weight" type="object" nullable={true}>
  <Description>The weight of the package.</Description>
</Field>

{' '}

<Field name="shipments[].packages[].weight.value" type="number" nullable={false}>
  <Description>The weight value for this package. This value may contain decimals.</Description>
</Field>
...
<Field name="shipments[].packages[].metadata" type="object" nullable={true}>
  <Description>
    The carrier's custom data about this package that was previously persisted by ShipEngine Connect. Must be JSON
    serializable.
  </Description>
</Field>
...
<Field name="cancellationId" required={true}>
  <Type>[UUID](https://www.npmjs.com/package/uuid)</Type>
  <Description>An identifier that indicates which pickup cancellation this outcome is for.</Description>
</Field>
...
<Field name="status" type="string" required={true}>
  <Description>
    The status of the cancellation request. Valid values include the following: * `success` - Cancellation was
    successful. * `error` - Cancellation encountered an error. * `timeout` - Cancellation call timed out, probably
    related to a network error. * `skipped` - Cancellation was skipped. * `throttled` - Cancellation could not be made
    because requests are being throttled.
  </Description>
</Field>
...
<Field name="description" type="string" required={false}>
  <Description>
    The carrier's description of the cancellation outcome. This description should not be specific to this particular
    pickup. This string must not contain newline characters.
  </Description>
</Field>
