...
One of the key benefits of integrating your service with ShipEngine Connect is that we will take care of hosting your application, ensuring that it scales to meet the
...
> **WARNING:** If at anytime you change the `name` property and re-publish, it will be treated as a different application.


### Definition IDs

[Definition](structure.mdx#definitions) `id` and `code` properties are used internally to uniquely identify your application's resources.
...
Only one [delivery confirmation](./reference/delivery-confirmation.mdx) definition file should be created for
