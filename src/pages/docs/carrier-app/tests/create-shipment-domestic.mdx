---
title: Automated Tests | createShipment_domestic
description: This test is part of the ShipEngine Connect carrier app test suite. It tests your app's ability to create a domestic shipment.
---
...
This test exercises your app's ability to successfully create a domestic shipment. If your app does not have any domestic [delivery services](../../reference/delivery-service.mdx), then this test will be skipped by [the `connect test` command](../../cli.mdx#test-your-app).
...
------------------------------
Here's an example of using the `connect.config.js` file to customize the `createShipment_domestic` test. This example provides values for all parameters and uses the global `connectArgs` parameter for authentication.
