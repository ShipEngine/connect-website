...
Usage
-----
Support for sandbox environments can be set in the application definitions. For example, in the [Delivery Service Definition](./v1/reference/delivery-service.mdx), you can specify this in the `hasSandbox` property.


Then when you implement the [createShipment method](./v1/reference/methods/create-shipment.mdx), ShipEngine Connect will indicate if the sandbox environment will be used by setting the `useSandbox` boolean in the [Transaction](./v1/reference/transaction.mdx) object that is included in every method call.
