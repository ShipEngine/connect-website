...
export function Menu({ contents }: MenuProps) {
  ...
  return (
    <nav className={styles.menu}>
      <button id="menu-button" className={styles.menuButton} onClick={() => setVisible(!visible)}></button>
      <ul className={`${styles.menuList} nav-menu ${visible ? 'visible' : ''}`}>{menu.map(Group)}</ul>
    </nav>
  )
}
...
function SubGroup({ title, selected, menuItems }: EnhancedSubGroup) {
  return (
    <li key={title}>
      <div className={`${styles.subGroup} ${selected ? styles.selected : ''}`}>{title}</div>
      <ul className={styles.subGroupList}>{menuItems.map(MenuItem)}</ul>
    </li>
  )
}
