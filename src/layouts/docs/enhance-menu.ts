...
export function enhanceMenu(
  ...
  for (const group of menu) {
    ...
    for (const subGroup of group.subGroups || []) {
      ...
      for (const menuItem of subGroup.menuItems) {
        ...
        enhancedSubGroup.selected =
          enhancedSubGroup.selected || enhancedItem.selected
    ...
    enhancedGroup.open = context.openedGroups[group.title] =
      context.openedGroups[group.title] || enhancedGroup.open
  ...
  function doMenuItem(menuItem: MenuItem): EnhancedItem {
    id++
    const selected =
      !hasSelectedAMenuItem && isSelected(id, menuItem.href, pathname, context)
