import {
  ...
  ContactInfoPOJO,
  NotePOJO,
  PickupCancellation as IPickupCancellation,
...
export class PickupCancellation implements IPickupCancellation {
  public static readonly [_internal] = {
    ...
    schema: Joi.object({
      ...
      shipments: Joi.array()
        .min(1)
        .items(PickupShipment[_internal].schema)
        .required(),
      metadata: Joi.object(),
    }),
  };
  ...
  public constructor(pojo: PickupCancellationPOJO, app: App) {
    ...
    this.id = pojo.id;
    this.pickupService = app[_internal].references.lookup(
      pojo.pickupService,
      PickupService,
    );
    ...
    this.shipments = pojo.shipments.map(
      (shipment) => new PickupShipment(shipment, app),
    );
