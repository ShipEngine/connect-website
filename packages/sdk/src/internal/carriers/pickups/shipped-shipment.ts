import {
  ...
  ShippedShipment as IShippedShipment,
} from '../../../public';
import {
  App,
...
export class ShippedShipment
  extends ShipmentIdentifierBase
  implements IShippedShipment
{
  public static readonly [_internal] = {
    ...
    schema: ShipmentIdentifier[_internal].schema.keys({
      ...
      packages: Joi.array()
        .min(1)
        .items(ShippedPackage[_internal].schema)
        .required(),
  ...
  public constructor(pojo: ShippedShipmentPOJO, app: App) {
    ...
    this.packages = pojo.packages.map(
      (parcel) => new ShippedPackage(parcel, app),
    );
