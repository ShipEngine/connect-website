 ...
 *
 */
export function getMaxServiceArea(
  things: ReadonlyArray<{ serviceArea?: ServiceArea }>,
): ServiceArea {
  let maxArea = 0;
  const serviceAreas = [
...
export function calculateTotalInsuranceAmount(
  packages: ReadonlyArray<{ insuredValue: MonetaryValue }>,
): MonetaryValue;
export function calculateTotalInsuranceAmount(
  packages: ReadonlyArray<{ insuredValue?: MonetaryValue }>,
...
export function calculateTotalInsuranceAmount(
  ...
  try {
    const insuredValues = packages
      .map((parcel) => parcel.insuredValue)
      .filter(Boolean) as MonetaryValue[];
