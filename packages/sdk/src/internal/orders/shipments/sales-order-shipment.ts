import {
  ...
  SalesOrderShipment as ISalesOrderShipment,
...
import {
  ...
  SalesOrderPackageItemPOJO,
} from './sales-order-package-item';
...
export class SalesOrderShipment
  extends ShipmentIdentifierBase
  implements ISalesOrderShipment
{
  public static readonly [_internal] = {
    label: 'shipment',
    schema: ShipmentIdentifier[_internal].schema.keys({
      trackingURL: Joi.alternatives(
        Joi.object().website(),
        Joi.string().website(),
      ),
      ...
      contents: Joi.array()
        .min(1)
        .items(SalesOrderPackageItem[_internal].schema)
        .required(),
  ...
  public constructor(pojo: SalesOrderShipmentPOJO) {
    ...
    this.trackingURL = pojo.trackingURL
      ? new URL(pojo.trackingURL as string)
      : undefined;
    ...
    this.contents = (pojo.contents || []).map(
      (item) => new SalesOrderPackageItem(item),
    );
    this.notifyBuyer = pojo.notifyBuyer;
    this.fulfillmentCost = pojo.fulfillmentCost
      ? new Charge(pojo.fulfillmentCost)
      : undefined;
    this.insuranceCost = pojo.insuranceCost
      ? new Charge(pojo.insuranceCost)
      : undefined;
