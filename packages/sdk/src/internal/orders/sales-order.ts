...
export class SalesOrder extends SalesOrderIdentifierBase {
  public static readonly [_internal] = {
    ...
    schema: SalesOrderIdentifier[_internal].schema.keys({
      ...
      requestedFulfillments: Joi.array()
        .min(1)
        .items(RequestedFulfillment[_internal].schema),
  ...
  public constructor(pojo: SalesOrderPOJO) {
    ...
    this.lastModifiedDateTime = pojo.lastModifiedDateTime
      ? new DateTimeZone(pojo.lastModifiedDateTime)
      : undefined;
    ...
    this.paymentAmount = pojo.paymentAmount
      ? new MonetaryValue(pojo.paymentAmount)
      : undefined;
    this.orderURL = pojo.orderURL
      ? new URL(pojo.orderURL as string)
      : undefined;
    this.buyer = new Buyer(pojo.buyer);

    this.charges = pojo.charges
      ? pojo.charges.map((charge) => new Charge(charge))
      : [];
    this.totalCharges = calculateTotalCharges(this.charges);

    this.requestedFulfillments = pojo.requestedFulfillments.map(
    ...
    this.originalOrderSource = pojo.originalOrderSource
      ? new OriginalOrderSource(pojo.originalOrderSource)
      : undefined;
