...
export class ReferenceMap {
  ...
  public add(instance: ClassInstance): void {
    ...
    if (existing) {
      ...
      if (isFinishedLoading) {
        ...
        throw error(
          ErrorCode.Invalid,
          `Cannot add new ${type[_internal].label} after the app has loaded`,
        );
      }
  ...
  public get<T extends ClassInstance>(
    instance: ClassInstance | string | undefined,
    type: ShipEngineConstructor<T>,
  ): T | undefined {
  ...
  public lookup<T extends ClassInstance>(
    ...
    if (!value) {
      if (typeof instance === 'string') {
        throw error(
          ErrorCode.Invalid,
          `Unable to find ${type[_internal].label}: ${instance}`,
        );
      } else {
        throw error(
          ErrorCode.Invalid,
          `Unable to find ${type[_internal].label} ID: ${instance.id}`,
        );
      }
    }
