...
describe("When parsing logs", () => {
  ...
  it("should filter out HTTP requests by default", () => {
    const parsedLogs = parseDIPLogs(sampleLogs, "1500", false);
  ...
  it("should parse a simple string", () => {
    const parsedLogs = parseDIPLogs("Simple log statement", "1500");
  ...
  it("should only return the number of lines specified by the flag", () => {
    const parsedLogs = parseDIPLogs(sampleLogs, "3", true);
