...
export default function getVersions(): Record<string, string> {
  ...
  const orderSourceRuntimePjson = require("@shipengine/connect-order-source-runtime/package.json") as PJson;

  const shippingRuntimePjson = require("@shipengine/connect-shipping-runtime/package.json") as PJson;
