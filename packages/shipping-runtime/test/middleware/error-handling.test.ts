...
import {
	getStandardizedErrorCode,
	StandardizedErrorCode,
	getStatusCode,
} from '../../src/middleware/error-handling';
...
describe('Error Handler', () => {
	describe('Map Standardized Error Codes', () => {
		test.each(expectedErrorCodeMapping)(
			'getStandardizedErrorCode(%s, %s) maps to %s',
	...
	describe('Map Status Codes', () => {
		test.each(expectedStatusCodeMapping)(
			'getStatusCode(%s, %s) maps to %s',
			(code, statusCode, expected) => {
