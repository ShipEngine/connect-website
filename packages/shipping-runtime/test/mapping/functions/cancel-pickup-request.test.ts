...
const expectedCancellationMappingResults: any[][] = [
	...
	[
		CancellationReason.CarrierFailedPickup,
...
describe('Cancel Pickup Request', () => {
	describe('Mapping Cancellation Reasons', () => {
		test.each(expectedCancellationMappingResults)(
			'mapCancellationReason(%s) maps to %s',
			(reason, expected) => {
	...
	describe('Mapping Cancel Pickup Request', () => {
		describe('when given only a transaction id', () => {
			...
			it('the transaction id is set as the cancellation id', () =>
				expect(request.cancellationID).toEqual('transaction'));
			it('the confirmation id is set to an empty string', () =>
				expect(request.id).toEqual(''));
			it('the pickup service is set to an empty string', () =>
				expect(request.pickupService).toEqual(''));
		});

		describe('when given a fully formed cancel pickup request', () => {
			it('maps correctly', () =>
				expect(mapCancelPickupRequest(fullyFormedRequest)).toEqual(
					fullyFormedExpectedMapping,
				));
		});
	});
	...
	describe('Identifier Reducer', () => {
		describe('when the identifiers are fully formed', () => {
			...
			it('it properly reduces an array of identifiers into the appropriate object', () =>
		...
		describe('when the identifiers are missing a field', () => {
			...
			const results = identifiers.reduce(identifierReducer, {});
			it('it properly reduces an array of identifiers into the appropriate object', () =>
				expect(results).toEqual({ item3: 'value3' }));
		});
	});
});
