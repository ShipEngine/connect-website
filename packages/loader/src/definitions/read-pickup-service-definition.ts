...
export async function readPickupServiceDefinition(
  definition: InlineOrReference<PickupServiceDefinition>,
  cwd: string,
  fieldName: string,
...
export async function readPickupServiceArrayDefinition(
  definitions: InlineOrReferenceArray<PickupServiceDefinition> | undefined,
  cwd: string,
  fieldName: string,
  ...
  if (Array.isArray(array)) {
    definitions = await Promise.all(
      array.map((service, index) =>
        readPickupServiceDefinition(service, cwd, `${fieldName}[${index}]`),
      ),
    );
