...
export async function readDeliveryServiceDefinition(
  definition: InlineOrReference<DeliveryServiceDefinition>,
  cwd: string,
  fieldName: string,
  ...
  return {
    ...definition,
    availableCountries: await readDefinitionValue(
      definition.availableCountries,
      cwd,
      `${fieldName}.availableCountries`,
    ),
    packaging: await readPackingArrayDefinition(
      definition.packaging,
      cwd,
      `${fieldName}.packaging`,
    ),
    deliveryConfirmations: await readDeliveryConfirmationArrayDefinition(
      definition.deliveryConfirmations,
      cwd,
      `${fieldName}.deliveryConfirmations`,
    ),
  };
}
...
export async function readDeliveryServiceArrayDefinition(
  definitions: InlineOrReferenceArray<DeliveryServiceDefinition>,
  cwd: string,
  fieldName: string,
): Promise<DeliveryServicePOJO[]> {
  ...
  if (Array.isArray(array)) {
    definitions = await Promise.all(
      array.map((service, index) =>
        readDeliveryServiceDefinition(service, cwd, `${fieldName}[${index}]`),
      ),
    );
  }
