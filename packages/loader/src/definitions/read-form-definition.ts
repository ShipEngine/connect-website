...
export async function readFormDefinition(
  definition: InlineOrReference<FormDefinition>,
  cwd: string,
  fieldName: string,
): Promise<FormPOJO>;
export async function readFormDefinition(
  definition: InlineOrReference<FormDefinition> | undefined,
  cwd: string,
  fieldName: string,
...
export async function readFormDefinition(
  ...
  if (definition) {
    return {
      ...definition,
      dataSchema: await readDefinitionValue(
        definition.dataSchema,
        cwd,
        `${fieldName}.dataSchema`,
      ),
      uiSchema: await readDefinitionValue(
        definition.uiSchema,
        cwd,
        `${fieldName}.uiSchema`,
      ),
