...
export async function readConnectionAppDefinition(
  definition: ConnectionAppDefinition,
  cwd: string,
  manifest: AppManifestPOJO,
): Promise<ConnectionAppPOJO> {
  return {
    ...
    connectionForm: await readFormDefinition(
      definition.connectionForm,
      cwd,
      'connectionForm',
    ),
    settingsForm: await readFormDefinition(
      definition.settingsForm,
      cwd,
      'settingsForm',
    ),
    connect: await readDefinitionValue(
      definition.connect,
      cwd,
      'connect method',
    ),
    oauthConfig: await readOAuthConfigDefinition(
