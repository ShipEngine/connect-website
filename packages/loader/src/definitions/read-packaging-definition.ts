...
export async function readPackagingDefinition(
  definition: InlineOrReference<PackagingDefinition>,
  cwd: string,
  fieldName: string,
): Promise<PackagingPOJO> {
...
export async function readPackingArrayDefinition(
  definitions: InlineOrReferenceArray<PackagingDefinition>,
  cwd: string,
  fieldName: string,
): Promise<PackagingPOJO[]> {
  ...
  if (Array.isArray(array)) {
    definitions = await Promise.all(
      array.map((packaging, index) =>
        readPackagingDefinition(packaging, cwd, `${fieldName}[${index}]`),
      ),
    );
  }
