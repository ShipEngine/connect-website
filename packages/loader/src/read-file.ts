...
export async function readFile<T>(filePath: string): Promise<T> {
  switch (path.extname(filePath)) {
    ...
    case '.ts':
      throw new Error(
        `Invalid file reference "${filePath}. Reference the compiled Javascript file instead of the Typescript source file.`,
      );
...
async function readTextFile(filePath: string): Promise<string> {
  try {
    ...
    throw error(SystemErrorCode.Filesystem, `Unable to read ${filePath}.`, {
      originalError,
    });
  }
}
