...
describe.only("ipaasLoader() with inline config", () => {

  beforeEach(async () => {
    inlineConfig = await ipaasLoader("../fixtures/integration-apps/carrier-inline-app");
  });

  it("should not attempt to dereference a config that has all properties inlined", () => {
    expect(inlineConfig.type).to.equal("shipping_provider");
    ...
    expect(inlineConfig.logo.colorSVG).to.be.instanceof(Buffer);
    expect(inlineConfig.logo.blackAndWhiteSVG).to.be.instanceof(Buffer);
