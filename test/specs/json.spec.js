...
describe("JSON apps", () => {
  ...
  it("should load a carrier app", async () => {
    let app = await loadApp("test/fixtures/apps/carrier/json");

    expect(app.manifest.name).to.equal("@carrier/json-app");
  ...
  it("should load a order app", async () => {
    let app = await loadApp("test/fixtures/apps/order/json");

    expect(app.manifest.name).to.equal("@order/json-app");
    expect(app.manifest.version).to.equal("0.0.1");
    expect(app.manifest.description).to.equal("");
