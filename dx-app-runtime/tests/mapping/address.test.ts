import {capiToDxAddress} from "../../src/mapping/functions/address";
...
describe("address mapping", () => {
    it("returns an empty DX address with null CAPI", () => {
        const dxAddress = capiToDxAddress(null);
    ...
    it("converts Address Residential Indicator to the correct boolean", () => {
        ...
        let dxAddress = capiToDxAddress(capiAddress);
        expect(dxAddress.isResidential).toBe(true);

        dxAddress = capiToDxAddress(capiAddress)
        capiAddress.address_residential_indicator = AddressResidentialIndicator.Yes;
        expect(dxAddress.isResidential).toBe(true);

        capiAddress.address_residential_indicator = AddressResidentialIndicator.Commercial;
        dxAddress = capiToDxAddress(capiAddress)
        ...
        capiAddress.address_residential_indicator = AddressResidentialIndicator.Unknown
        dxAddress = capiToDxAddress(capiAddress)
        expect(dxAddress.isResidential).toBe(false);
    })
})
