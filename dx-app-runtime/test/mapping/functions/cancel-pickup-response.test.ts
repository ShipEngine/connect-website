...
describe('Cancel Pickup Response', () => {
    describe('when given a fully formed successful pickup cancellation outcome', () => {
        ...
        it('it maps confirmation_id correctly', () => expect(result.confirmation_id).toEqual('CONF1'));
        it('it maps successful correctly', () => expect(result.successful).toEqual(true));
        it('it maps status correctly', () => expect(result.status).toEqual('description'));
        it('it maps custom_properties correctly', () => expect(result.custom_properties).toEqual({carrier_code: 'CODE1'}));
        it('it maps metadata correctly', () => expect(result.metadata).toEqual({ value: 1 }));
    });
})