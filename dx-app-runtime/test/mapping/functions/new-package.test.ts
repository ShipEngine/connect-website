...
describe('New Package', () => {
	describe('Insured Values Map Correctly', () => {
		test.each(expectedInsuranceMapping)(
			'mapInsuredValue(%s, %s) maps to %s',
			(insuranceProvider, currency, expected) => {
	...
	describe('Non Delivery Options Map Properly', () => {
		test.each(expectedNonDeliveryOptionsMapping)(
			'mapNonDeliveryOption(%s) maps to %s',
			(option, expected) => {
	...
	describe('New Label Maps Correctly', () => {
		test.each(expectedReferenceMapping)(
			"mapNewLabelPOJO('pdf', '4x6', %s) maps to %s",
			(references, expected) => {
				expect(
					mapNewLabelPOJO(
						DocumentFormat.PDF,
	...
	describe('Customs Maps Properly', () => {
		it('it should be undefined when the customs item is undefined.', () =>
			expect(mapCustomsPOJO()).toEqual(undefined));
		it('it should map to undefined if no items are present', () =>
			expect(mapCustomsPOJO(customsWithNoItems)).toEqual(undefined));
		describe('when it has a full customs item', () => {
			const result = mapCustomsPOJO(customs);
			it('it should map non delivery options properly', () =>
				expect(result?.nonDeliveryOption).toEqual('return'));
			it('it should have the proper number of contents', () =>
	...
	describe('when we have a fully filled out package', () => {
		...
		it('it maps nothing to contents because that does not exist in capi', () =>
			expect(result.contents).toEqual(undefined));
		it('it maps isNonMachinable correctly', () =>
			expect(result.isNonMachinable).toEqual(true));
		it('it maps packaging correctly', () =>
			expect(result.packaging).toEqual('packageCode'));
	});

	describe('nonEmptyCustomsItemsFilter', () => {
		it('the filter should return false when the item is undefined', () =>
			expect(nonEmptyCustomsItemsFilter(undefined)).toEqual(false));
		it('the filter should return false when the item is an empty object', () =>
			expect(nonEmptyCustomsItemsFilter({})).toEqual(false));
		it('the filter should return false when quantity is 0', () =>
			expect(nonEmptyCustomsItemsFilter({ quantity: 0 })).toEqual(false));
		it('the filter should return true when description is specified', () =>
			expect(nonEmptyCustomsItemsFilter({ description: 'value' })).toEqual(
				true,
			));
		it('the filter should return true when harmonized_tariff_code is specified', () =>
			expect(
				nonEmptyCustomsItemsFilter({ harmonized_tariff_code: 'value' }),
			).toEqual(true));
		it('the filter should return true when quantity is greater than 0', () =>
			expect(nonEmptyCustomsItemsFilter({ quantity: 1 })).toEqual(true));
		it('the filter should return true when sku is specified', () =>
			expect(nonEmptyCustomsItemsFilter({ sku: 'value' })).toEqual(true));
		it('the filter should return true when sku_description is specified', () =>
			expect(nonEmptyCustomsItemsFilter({ sku_description: 'value' })).toEqual(
				true,
			));
	});
});
