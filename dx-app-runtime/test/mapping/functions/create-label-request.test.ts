...
import { mapCreateLabelRequest, getReturnToAddress } from '../../../src/mapping/functions';
...
const expectedReturnAddress: any[][] = [
    ...
    [true, labelRequest.ship_from, labelRequest.ship_from_display, shipFromDisplayAddress],
]


describe('Create Label Request', () => {
    describe('delivery services', () => {
        it('when the service_code is undefined it maps deliveryService to an empty string', () => expect(mapCreateLabelRequest({ ...labelRequest, service_code: undefined }).deliveryService).toEqual(''));
        it('when the service_code is defined it maps correctly to deliveryService', () => expect(mapCreateLabelRequest(labelRequest).deliveryService).toEqual('service_code'));
    });
    describe('return address is mapped correctly', () => {
        test.each(expectedReturnAddress)('getReturnToAddress(%s, %o, %o) returns %o', (isReturn, from, display, expected) => {
