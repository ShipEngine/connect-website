import { PickupConfirmationPOJO, ShipmentIdentifierPOJO, NotePOJO, TransactionPOJO } from "@shipengine/integration-platform-sdk";
import { SchedulePickupResponse } from "@ipaas/capi/responses";
import { ShipmentIdentifier, Identifier as capiIdentifier, PickupWindow } from '@ipaas/capi/models';

const mapIdentifier = (key: string, value: string | undefined): capiIdentifier => {
  return {
    type: key,
    value: value
  };
...
const mapNotesToString = (notes: string | readonly(string | NotePOJO)[]): string => {
  let combinedNotes = '';
...
const mapShipmentIdentifiers = (shipmentIdentifier: ShipmentIdentifierPOJO): ShipmentIdentifier => {
  ...
  if(shipmentIdentifier.identifiers) {
    ...
    keys.forEach(key => {
      const value = shipmentIdentifier.identifiers ? shipmentIdentifier.identifiers[key] : undefined;
...
export const mapPickupConfirmationPOJOToSchedulePickupResponse = (response: PickupConfirmationPOJO, transaction: TransactionPOJO): SchedulePickupResponse => {
