...
import {  PickupCancellationReason, TimeRangePOJO, PickupCancellationPOJO, IdentifiersPOJO } from "@shipengine/integration-platform-sdk";
import { CancellationReason,
         PickupWindow,
         Identifier as capiIdentifier } from "@ipaas/capi/models";
...
const mapPickupWindow = (pickupWindow: PickupWindow | null | undefined): TimeRangePOJO => {
...
export const mapCancelPickupRequestToPickupCancellationPOJO = (request: CancelPickupRequest): PickupCancellationPOJO => {
  return {
    ...
    identifiers: request.confirmation?.alternate_identifiers?.reduce(mapIdentifier, {}) || undefined,
    reason: mapCancellationReason(request.cancellation_details?.reason),
    notes: request.cancellation_details?.remarks || undefined,
    address: mapAddressToAddressWithContactInfoPOJO(request.location?.pickup_address),
