...
export const mapInsuredValue = (provider?: InsuranceProvider, insuredValue?: Currency): MonetaryValuePOJO | undefined => {
...
export const mapNewLabelPOJO = (format: DocumentFormat, size: DocumentSize, messages?: LabelMessages): NewLabelPOJO => {
  if(!messages || (!messages.reference1 && !messages.reference2 && !messages.reference3)) {
    return {
      format,
      size
    }
...
export const mapNonDeliveryOption = (option?: NonDelivery): NonDeliveryOption | undefined => {
  switch(option) {
...
export const mapNewPackage = (
    ...
    const mappedPackage: NewPackagePOJO = {
      ...
      insuredValue: mapInsuredValue(insuranceProvider, capiPackage?.insured_value),
      containsAlcohol: advancedOptions?.contains_alcohol || false,
      isNonMachinable: advancedOptions?.nonmachineable || false,
      label: mapNewLabelPOJO(documentFormat, documentSize, capiPackage.label_messages || undefined),
    };
