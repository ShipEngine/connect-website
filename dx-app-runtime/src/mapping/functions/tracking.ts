...
const capiToDxStandardizedStatusCode = (shipmentStatus: ShipmentStatus): StandardizedStatusCodes => {
    ...
    throw new Error(`capi shipment status ${shipmentStatus} is not convertible to DX StandardizedStatusCode`);
}
...
const dxToCapiTrack = (trackingInfo: TrackingInfo, transaction: TransactionPOJO): TrackResponse => {
    ...
    const capiInfo: CapiTrackingInfo = {
        ...
        standardized_status_code: capiToDxStandardizedStatusCode(trackingInfo.status),
