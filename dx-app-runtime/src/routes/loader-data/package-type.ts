...
import PackageType, {PackageAttribute} from "../../mapping/registry-data/external/package-type";
...
const serviceAreaToPackageAttribute = (serviceArea: ServiceArea): PackageAttribute => {
  switch (serviceArea) {
    ...
    case ServiceArea.Global:
      ...
      break;
    default:
      throw new MappingError(`unknown service area ${serviceArea}`, {
...
const resolvePackageAttributes = (packaging: Packaging, services: ReadonlyArray<DeliveryService>): PackageAttribute[] => {

  const packagingId = packaging.id;

  const distinctServiceAreas = [... new Set(services.filter(service =>
...
const dxToCapiSpecPackageType = (packaging: readonly Packaging[], services: ReadonlyArray<DeliveryService>): PackageType[] => {
  const packageTypes: PackageType[] = [];
  packaging.forEach((dxPackage) => {
    if (dxPackage.name.toLowerCase() === "package") {
      throw new ValidationError(`PackageId ${dxPackage.id} can not be named ${dxPackage.name}`, "Packaging.Name", dxPackage.name)
    }

    const packageAttributes = resolvePackageAttributes(dxPackage, services);
    if(!packageAttributes || packageAttributes.length === 0){
      throw new ValidationError(`Could not resolve package attribute (domestic/int'l) for PackageId ${dxPackage.id}`,
        "Packaging",
        dxPackage.id);
    }
